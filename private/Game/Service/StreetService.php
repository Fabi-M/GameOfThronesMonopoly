...
class StreetService
{
    ...
    {
        if (!((bool) $this->game->getGameEntity()->getAllowedToEndTurn())) {
        ...
        if (!$this->checkIfBuyable()) {
            throw new Exception("Street is already owned");
        } // street is already owned by another player
        if (!$this->player->buyStreet($this->em)) {
            throw new Exception("Player doesn't have enough money");
    ...
    public function sellStreet($fieldId)
    {
        ...
        if ($this->playerXField == null) {
            throw new Exception("This street is currently not owned, you can't sell the street");
        } // doesn't own street
        if ($this->playerXField->getPlayerXStreetEntity()->getBuildings() > 0) {
            throw new Exception("You have to sell the houses before you can sell the street");
        } // has houses on street, needs to sell them first
    ...
    public function buyHouse($fieldId, EntityManager $em): bool|array
    {
        ...
        $this->playerXField->getPlayerXStreetEntity()->setBuildings(
            $this->playerXField->getPlayerXStreetEntity()->getBuildings() + 1
        );
     ...
     * @author Fabian Müller
     */
    public function checkIfFullStreet(?Player $player = null, ?Street $street = null): bool
    {
        if (is_null($player)) {
            $player = $this->player;
        }
        ...
        return filter_var(
            StreetRepository::checkIfAllStreetsOwned(
                $street->getStreetEntity()->getColor(),
                $player->getPlayerEntity()->getId()
            ),
    ...
    public function getAllModels($fieldId)
    {
        ...
        $this->playerXField = PlayerXStreetFactory::getByFieldId(
            $this->em, $this->game->getGameEntity()->getId(), $fieldId
        );
    }
}